 ...
 * current time
 */
int64_t getPressurePartB(int64_t nMyStartIdx,
        int64_t nElStartIdx,
        int64_t nMyCurrTime,
        int64_t nEleCurrTime,
        int64_t *nAlreadyOpened);
...
int main(void)
{
    ...
    cout << "Part (b) possible solution(s): (please keep waiting)" << endl;
    /* Keep displaying all the pressure values that are larger than previous
     * and keep trying it on the portal one by one */
    cout << getPressurePartB(gnIdxAA, gnIdxAA, 26, 26, &nTempVec[0]) << endl;
...
/* nLevel indicates the depth of recursion
 * nLevel of 1 indicates that the function is just called
 * nLevel of 2 indicates that the recursive function is called from the function
 * again, and so on */
int64_t nLevel = 0;

/* Recursive function to get the total pressure from starting index and
 * current time
 * nMyStartIdx - Starting index of my valve
 * nElStartIdx - Starting index of elephant's valve
 * nMyCurrTime - My time remaining
 * nElCurrTime - Elephant's time remaining
        ...
        int64_t *nAlreadyOpened)
{
    ...
    /* Increment the level as the function is now called */
    ++nLevel;

    /* If the time has been elapsed, return the pressure as zero as no valve
     * can be open after time runs out */
    if (nElCurrTime <= 0 && nMyCurrTime <= 0)
    {
        /* Decrement the level before exiting from the function */
        --nLevel;
        return nMaxPressure;
    }
    ...
    for (nCnt1 = 0; nCnt1 < gnNonZeroRate.size(); nCnt1++)
    {
        ...
        /* Remaining time for me */
        int64_t nMyRemainingTime = 0;
        /* Remaining time for the elephant */
        int64_t nElRemainingTime = 0;

        /* Loop through ll the values of non-zero rate once again for the
         * elephant and move to a non-zero rate next, to open the valve */
        for (nCnt2 = 0; nCnt2 < gnNonZeroRate.size(); nCnt2++)
        {
            /* Current pressure is zero */
            nPressure = 0;
            /* If the valve is already opened by the me/elephant skip and move
             * on tot he next valve */
            if (nAlreadyOpened[gnNonZeroRate[nCnt2]] == 1 || nCnt1 == nCnt2)
            {
                continue;
            }
                ...
                (nElCurrTime > (gnShortCost[nElStartIdx][gnNonZeroRate[nCnt2]] + 1)))
            {
                ...
                if (nElRemainingTime != -1)
                {
                    nAlreadyOpened[gnNonZeroRate[nCnt2]] = 0;
                }
            }
            ...
            if (nMaxPressure < nPressure)
            {
                ...
                /* If there is an update on the top level function, display it */
                if (nLevel == 1)
                {
                    cout << "Try: " << nMaxPressure << endl;
                }
            }
        }
    }

    /* Decrement the level before exiting */
    --nLevel;
